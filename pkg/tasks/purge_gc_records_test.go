...
func TestPurgeGCRecordsTable_Create(t *testing.T) {
	t.Run("creates a gc record", func(t *testing.T) {
		badgerStore, err := store.NewBadgerInMemoryStore()
		require.NoError(t, err)

		table := newPurgeGCRecordsTable([]byte{0x77, 0x77, 0x77, 0x77})
		...
		err = table.Create(txn, &corepb.PurgeQueueGarbageCollectionRecord{QueueId: queueId})
		require.NoError(t, err)
	...
	t.Run("creates multiple gc records", func(t *testing.T) {
		...
		txn := badgerStore.Update()
		for i := range 5 {
			err = table.Create(txn, &corepb.PurgeQueueGarbageCollectionRecord{
...
func TestPurgeGCRecordsTable_Delete(t *testing.T) {
	...
	t.Run("deletes one of multiple gc records", func(t *testing.T) {
		...
		records := make([]*corepb.PurgeQueueGarbageCollectionRecord, 3)
		txn := badgerStore.Update()
		for i := range 3 {
			records[i] = &corepb.PurgeQueueGarbageCollectionRecord{
				QueueId: &corepb.QueueId{AccountId: accountId, QueueId: uint64(i + 1)},
			}
...
func TestPurgeGCRecordsTable_Exists(t *testing.T) {
	t.Run("reports true for a marked queue id", func(t *testing.T) {
		badgerStore, err := store.NewBadgerInMemoryStore()
		require.NoError(t, err)
		...
		exists, err := table.Exists(txn, accountId, queueId)
		require.NoError(t, err)
		require.True(t, exists)
	})

	t.Run("reports false for a queue id that was never marked", func(t *testing.T) {
		badgerStore, err := store.NewBadgerInMemoryStore()
		require.NoError(t, err)
	...
	t.Run("reports false again once the marker is deleted", func(t *testing.T) {
		badgerStore, err := store.NewBadgerInMemoryStore()
		require.NoError(t, err)
		...
		txn := badgerStore.Update()
		require.NoError(t, table.Create(txn, record))
		require.NoError(t, txn.Commit())

		txn = badgerStore.Update()
		require.NoError(t, table.Delete(txn, record))
		require.NoError(t, txn.Commit())

		txn = badgerStore.View()
		defer txn.Discard()

		exists, err := table.Exists(txn, record.QueueId.AccountId, record.QueueId.QueueId)
		require.NoError(t, err)
		require.False(t, exists)
	})

	t.Run("does not confuse different queue ids under the same account", func(t *testing.T) {
		badgerStore, err := store.NewBadgerInMemoryStore()
		require.NoError(t, err)
		...
		accountId := rand.Uint64()
		markedQueueId, unmarkedQueueId := rand.Uint64(), rand.Uint64()

		txn := badgerStore.Update()
		err = table.Create(txn, &corepb.PurgeQueueGarbageCollectionRecord{
			QueueId: &corepb.QueueId{AccountId: accountId, QueueId: markedQueueId},
		})
		require.NoError(t, err)
		require.NoError(t, txn.Commit())
		...
		exists, err := table.Exists(txn, accountId, markedQueueId)
		require.NoError(t, err)
		require.True(t, exists)

		exists, err = table.Exists(txn, accountId, unmarkedQueueId)
		require.NoError(t, err)
		require.False(t, exists)
	})
}

func TestPurgeGCRecordsTable_List(t *testing.T) {
